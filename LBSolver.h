...
#include"DGSolver.h"
#include"basefun.h"
class LBSolver : public DGSolver {
public:
	LBSolver(int Nxx, int Nyy, int Nzz) :DGSolver(Nxx, Nyy, Nzz) {}
	~LBSolver()
	{
...
{
	fftw_plan_initial();
	LBSolver::DGSolver::ProjMatrix(ProjMatrix);
	LBSolver::DGSolver::get_kspace(ProjMatrix, pk, pk2, kt, ktmp, kt_gradient);
	LBSolver::DGSolver::get_initial_value(Phi_hat);
